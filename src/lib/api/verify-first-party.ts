...
/**
 * Resms Verify for Resms's OWN consumer surfaces.
 *
 ...
 * our own consumer verification on Resms Verify removes that cost and the
 ...
 * for destinations Resms Verify does not serve.
 */
...
export type FirstPartySendResult =
  ...
  /** Resms Verify does not serve this destination - caller should fall back. */
 ...
 * purpose: the first two mean "Resms Verify can't do this one, use the old
...
export async function sendFirstPartyVerification(input: {
  ...
  // so Resms Verify does not serve them - but a real consumer with a Caribbean
