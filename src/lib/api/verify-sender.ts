 ...
 *
 * Pool numbers are Ghost-owned and sent under Resms's own 10DLC verification
 ...
 * deploy), then the RESMS_VERIFY_SENDER_POOL env var.
 */
export async function getSenderPool(): Promise<string[]> {
  ...
  if (numbers.length === 0) {
    // DELIVERED_VERIFY_SENDER_POOL is what production Vercel still sets; read
    // it until the env var is renamed there, then keep it as a fallback.
    numbers = (process.env.RESMS_VERIFY_SENDER_POOL || process.env.DELIVERED_VERIFY_SENDER_POOL || '')
      .split(',')
  ...
  if (input.requestedFrom) {
    if (!activeNumbers(input.tenant).includes(input.requestedFrom)) {
      throw new NoSenderAvailableError(
        '`from` must be a number your account owns. Leave it out and Resms will send from its verification pool.'
      );
    }
