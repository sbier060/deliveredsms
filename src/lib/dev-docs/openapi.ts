  ...
  components: {
    securitySchemes: {
      apiKey: {
        type: 'http',
        scheme: 'bearer',
        description: 'Your API key, e.g. Authorization: Bearer ghost_sk_test_...',
      },
    },
    schemas: {
      ...
      Event: {
        ...
        properties: {
          ...
          type: {
            type: 'string',
            enum: ['message.sent', 'message.delivered', 'message.failed', 'message.received', 'number.purchased', 'number.released', 'verification.sent', 'verification.approved', 'verification.failed', 'verification.blocked'],
          },
          created_at: { type: 'string', format: 'date-time' },
          data: { type: 'object' },
        },
