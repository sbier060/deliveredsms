...
/**
 * OpenSMS MCP server — Streamable HTTP transport in stateless JSON mode
...
const TOOLS = [
  {
    name: 'send_message',
    description:
      'Send an SMS from one of your OpenSMS numbers. In sandbox (test keys), delivery is simulated — +15005550006 delivers, +15005550002 fails.',
    inputSchema: {
      type: 'object',
  ...
  },
  {
    ...
    description:
      'Send a one-time verification code (OTP) to a phone number. OpenSMS generates the code, handles expiry (10 min) and attempt limits (5), and blocks SMS-pumping fraud. Prefer this over building OTP on send_message.',
    inputSchema: {
      type: 'object',
      required: ['phone'],
      properties: {
  ...
  {
    name: 'lookup_spam',
    description: "Get the aggregate spam signal for a number from OpenSMS's detection graph.",
    inputSchema: {
      type: 'object',
      required: ['phone'],
      properties: { phone: { type: 'string' } },
