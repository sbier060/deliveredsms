...
- API: `api.opensms.dev/v1` · MCP: `mcp.opensms.dev`
- SDK + CLI: [`packages/opensms`](packages/opensms) → npm `opensms`
...
- `src/lib/urls.ts` is the single source of hostnames. No URL literals
  anywhere else — `grep -r joinghostapp` and `grep -rE "https://[a-z]+\.opensms" src`
  outside that file should stay empty.
- API keys: `osms_sk_*` minted; legacy `ghost_sk_*` accepted forever.
